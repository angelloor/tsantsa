...
export const view_course = (course: Course, id_company: string) => {
	return new Promise<Course[]>(async (resolve, reject) => {
		const query = `select ${COLUMNS_RETURN} from business.view_course bvc${INNERS_JOIN}${
			course.name_course != 'query-all'
				? ` where lower(bvc.name_course) LIKE '%${course.name_course}%' and bvc.id_company = ${id_company}`
...
export const view_course_specific_read = (
	course: Course,
	id_company: string
) => {
	return new Promise<Course[]>(async (resolve, reject) => {
		const query = `select ${COLUMNS_RETURN} from business.view_course bvc ${INNERS_JOIN} where bvc.id_course = ${course.id_course} and bvc.id_company = ${id_company}`;
 ...
 * Reports
 */
export const view_course_by_period_read = (
	course: Course,
	id_company: string
) => {
	return new Promise<Course[]>(async (resolve, reject) => {
		const query = `select ${COLUMNS_RETURN} from business.view_course bvc ${INNERS_JOIN} where bvc.id_period = ${course.period.id_period} and bvc.id_company = ${id_company}`;
