 ...
 */
const COLUMNS_RETURN: string = `bvp.id_period, bvp.id_company, bvp.name_period, bvp.description_period, bvp.start_date_period, bvp.end_date_period, bvp.maximum_rating, bvp.approval_note_period, bvp.deleted_period, cvc.name_company, cvc.acronym_company, cvc.address_company, cvc.status_company`;
...
export const view_period_specific_read = (period: Period) => {
	return new Promise<Period[]>(async (resolve, reject) => {
		const query = `select ${COLUMNS_RETURN} from business.view_period bvp ${INNERS_JOIN} where bvp.id_period = ${period.id_period}`;
