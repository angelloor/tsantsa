...
export class NewsletterListComponent implements OnInit {
  ...
  ngOnInit(): void {
     ...
     * Get the newsletters
     */
    this.newsletters$ = this._newsletterService.newsletters$;
    /**
    ...
    this.searchInputControl.valueChanges
      .pipe(
        ...
        switchMap((query) => {
           ...
           * Search
           */
          return this._newsletterService.readNewsletterByQuery(
            query.toLowerCase()
          );
        })
      )
  ...
  createNewsletter(): void {
    ...
    this._angelConfirmationService
      .open({
        ...
        message:
          '¿Estás seguro de que deseas añadir una nueva anuncios? ¡Esta acción no se puede deshacer!',
      })
      .afterClosed()
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe((confirm: ActionAngelConfirmation) => {
        if (confirm === 'confirmed') {
           ...
           */
          this._newsletterService
            .createNewsletter(id_user_, id_company)
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe({
              next: (_newsletter: Newsletter) => {
                if (_newsletter) {
                  this._notificationService.success(
                    'Anuncios agregada correctamente'
                  );
                  /**
                   * Go to new anuncios
                   */
                  this.goToEntity(_newsletter.id_newsletter);
                } else {
